...
func main() {
	...
	for i := 0; i < n-1; i++ {
		a := aList[i]
		if i == n -2{
			if dp[i+1] >= dp[i] +int(math.Abs(float64(a-aList[i+1]))){
				dp[i+1] = dp[i] + int(math.Abs(float64(a-aList[i+1])))
			}
			break
		}
		...
		if dp[i+2] >=dp[i] + int(math.Abs(float64(a-aList[i+2]))){
			dp[i+2] = dp[i] + int(math.Abs(float64(a-aList[i+2])))
		}
