...
public class Main {
    ...
    static class Task {
        void solve(Scanner sc, PrintWriter out) {
            ...
            for (int i = 1; i < N + M; i++) {
                int j = i - M;
                double num1 = (j >= 1 && j<= N-1) ? f(i - M, N, R) : 0;
                double num2 = (i <= N-1) ? f(i, N, R) : 0;
                ans += Math.max(num1, num2);
            }
            out.println(ans);

        }
        static double f(int i, int N, int R){
            double tmp1 = Math.abs(1.0*N/2 - i) * 2.0 * R / N;
            return 2 * Math.sqrt(Math.pow(R, 2) - Math.pow(tmp1, 2));
