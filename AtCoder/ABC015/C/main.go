...
func main() {
	...
	answers := make([][]int, n)
	for i := 0; i < n; i++ {
		answers[i] = getIntSlice(k)
	}
	if dfs(n, k, 0, 0, answers){
		fmt.Println("Found")
	}else{
		fmt.Println("Nothing")
	}
...
func dfs(num int, kind int,  ques int, value int, answers [][]int) bool{
	if ques == num{
		return value == 0
	}

	for i := 0; i<kind;i++ {
		if dfs(num, kind, ques+1, value ^ answers[ques][i], answers){
			return true
		}
	}
